...
try {
	...
	if( preg_match("/\b(?:(?:https?|ftp|http):\/\/|www\.)[-a-z0-9+&@#\/%?=~_|!:,.;]*[-a-z0-9+&@#\/%=~_|]/i", $_POST["mensaje"]) ){
		echo json_encode(array('status' => false, 'message' => "Favor de no ingresar links o enlaces externos en la forma de contacto."));
		exit;
	}
   ...
   if ($data['success'] == 1 && $data['score'] >= 0.6) { 
		...
		if(empty($_POST)){
			echo json_encode(array('status' => false, 'message' => "No se ha podido enviar los datos"));
		} else {
			$emailResp = @mail($to, $title, $message, $headers);
			echo json_encode(array('status' => true, 'message' => "La forma de contacto se ha enviado con exito."));
		}
   } else {
      echo json_encode(array('status' => false, 'message' => 'No se podido ha procesar tu solicitud por posible SPAM, intenta más tarde.'));
   }
} catch (Exception $e) {
